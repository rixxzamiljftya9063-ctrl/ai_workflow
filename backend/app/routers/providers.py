...
from app.models.entities import ApiProvider, Project
...
def list_providers(project_id: int, db: Session = Depends(get_db)):
...
def create_provider(project_id: int, payload: ApiProviderCreate, db: Session = Depends(get_db)):
...
def update_provider(provider_id: int, payload: ApiProviderUpdate, db: Session = Depends(get_db)):
...
async def chat_provider(provider_id: int, payload: ApiProviderChatRequest, db: Session = Depends(get_db)):
    provider = db.get(ApiProvider, provider_id)
    if not provider:
        raise HTTPException(status_code=404, detail="Provider not found")
