...
from app.models.entities import Workflow, WorkflowRun, WorkflowRunStep
...
async def run_workflow(workflow_id: int, payload: dict[str, Any] | None = Body(default=None), db: Session = Depends(get_db)):
...
async def run_workflow_node_async(workflow_id: int, node_id: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    workflow = db.get(Workflow, workflow_id)
    if not workflow:
        raise HTTPException(status_code=404, detail="Workflow not found")
    run = WorkflowRunner(db).create_run_record(workflow)
...
async def run_workflow_node(workflow_id: int, node_id: str, db: Session = Depends(get_db)):
    workflow = db.get(Workflow, workflow_id)
    if not workflow:
        raise HTTPException(status_code=404, detail="Workflow not found")
